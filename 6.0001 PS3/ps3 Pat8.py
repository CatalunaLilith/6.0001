    ...
    if sorted(shared_ele) == sorted(word):
        if word not in word_list:
            ...
            if (word.find("*")) != -1:
                ...
                for char in VOWELS:
                    mod_word = word.replace("*", char)
                    if mod_word in word_list:
                        break
            else:
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while hand_num > 0:
        ...
        #if has_sub==False then ask if want substitute_hand
        if has_sub == False:
            print("Would you like to substitute a letter? yes or no")
            want_sub = input()
            if want_sub == "yes": 
                print("Which letter from your hand would you like to replace:")
                letter = input()
        ...
        #if has_replay==False then ask if want replay hand
        if has_replay == False:
            print("Would you like to replay the hand? yes or no")
            want_replay = input()
            #if no, move onto next hand (out of loop)
            if want_replay == "yes": 
                #store temp_hand_score1 = hand_score
                temp_hand_score1 = hand_score
                #replay hand, store temp_hand_score2 
                temp_hand_score2 = play_hand(hand, word_list)
                #hand_score is higher of temp_hand_score1 or temp_hand_score2
                hand_score = max(temp_hand_score1, temp_hand_score2)
                has_replay=True
        
        #add hand_score to total_score
